...
var BlitzMap = new function(){
    ...
    this.init = function() {
        ...
        if( isEditable ){  // changed by cheng
...
//                + GetInfoWindowFooter( overlay );
    	// added by cheng
    	var content ='<div>'+overlay.title+'</div>';
        return content;
    }
    ...
    function openInfowindow( overlay, latLng, content ){
    	...
    	if( overlay.type == 'polygon' || overlay.type == 'circle' || overlay.type == 'rectangle'  ){ // added by cheng
    		  var div = document.createElement('div');
    	        ...
    	        //var t = overlay.get( 'fillColor' ); // removed by cheng
//    	        infWindow.setMap( mapObj );   // Cheng
    ...
    function getEditorContent( overlay ){
		...
		*/
    	 ...
    	 // added by cheng
        var overlay_title = overlay.title;
    		var content = '<style>'
       		 + '#BlitzMapInfoWindow_container { width:250px; height:85px} '
                + 'div.infoWinSelType {margin: 0 0 0 0} '
                + 'div.infoWinButton {width:250px;display:inline-block;overflow: auto; margin-top:0px; white-space: nowrap; } '
                + 'input.floatLeft {float: left;} '
                + 'input.floatRight {float: right;} '
                + 'div.infoWinSelType label {display: block; padding-left: 17px; text-indent: -15px;} '
                + 'div.infoWinSelType input { width: 13px;  height: 13px;  padding: 0; margin:0 1 0 0;vertical-align: middle; position: relative; top: -1px; *overflow: hidden;} '
                + 'div.infoWinButton input {font-size: 106%; width: 48px; color: black;} '
                + '</style>'
                + '<form style="height:100%"><div id="BlitzMapInfoWindow_container" style="height:100%">'
                + '<div id="BlitzMapInfoWindow_details">';
       	 if( overlay.type == 'polygon'  ){
       		 content += '<div class="infoWinSelType"><label>';
       		 
       		 if(overlay_title === 'Destination out with the catchment area'){
       			 content += '<input type="radio" name="areaType" value="Catchment area">Catchment area';
           		 content += '</label><label>';
           		 content += '<input type="radio" name="areaType" value="Destination out with the catchment area" checked>Destination out with the catchment area';
           	}else{
           		content += '<input type="radio" name="areaType" value="Catchment area" checked>Catchment area';
          		content += '</label><label>';
          		content += '<input type="radio" name="areaType" value="Destination out with the catchment area" >Destination out with the catchment area';
       		 }
       		content += '</label></div>';
       	 }
       	 content += '<div class="infoWinButton"><input type="button" name="btn_del" class="floatLeft" value="Delete" onclick="BlitzMap.deleteOverlay()"  title"Delete selected shape">'
                +  '<input type="button" name="btn_cancel" value="Cancel" class="floatRight" style="margin-left: 3px;"onclick="this.form.reset();BlitzMap.closeInfoWindow()" >'
                +  '<input type="button" name="btn_ok" value="OK" class="floatRight" onclick="BlitzMap.closeInfoWindow()"  title="Apply changes to the overlay">';
    ...
    this.updateOverlay = function(){
        //infWindow.relatedOverlay.title = document.getElementById( 'BlitzMapInfoWindow_title' ).value; // commented by cheng
        //infWindow.relatedOverlay.content = document.getElementById( 'BlitzMapInfoWindow_content' ).value; // commented by cheng
    	infWindow.relatedOverlay.title = $("input[name=areaType]:checked").val(); // added by cheng
    	//infWindow.relatedOverlay.setEditable(false);  // test by cheng
        /*
         * old stuff
         * */
    	/*
