$(document).ready(function() {
	...
	//	    	  $('#'+passenger_elig_id).hide();
	   ...
	   $('select[name="other_elig_type"]').change(function() {
		   var value = $(this).val(); 
   ...
   $("input[name='fare_dist1_mile_2']").change(function() {
	   var value = $(this).val(); 
	   $("input[name='fare_dist2_mile_1']").val(value);
	   $("input[name='fare_dist2_mile_1']").attr('readonly', true);
	  });
...
$(document).ready(function() {
	$('#data_input_form_update')
	.bootstrapValidator({
		excluded: [':hidden'], //':disabled',
	    message: 'This value is not valid',
	    submitHandler: function(validator, form, submitButton) {
//	    	console.log($('#surcharge_table').html());
//	    	console.log($('#fare_table').html());
	    	allInputAble();
	        ...
	        return_fare_multiplier: {
	            validators: {
	                between: {
	                    min: 0,
	                    max: 2,
	                    message: '*&nbsp;The return fare multiplier must be greater than 0 but less than 2.0.'
	                }
	            }
	        },
	        discount_for_over60: {
...
//	        	}
        		...
        		var added_ind = parseInt(ind);
        		var div_html = '';
        		div_html += '<div class="row addedMoreSNA" id = "addedMoreSNA'+(added_ind+1)+'" style="margin-top:10px;">';
...
//	    	});
	...
	$( "#edit_general_info" ).click(function() {
		...
		if(!editable_gen_info){
			...
			$( "#edit_general_info" ).html("Confirm changes");
		}else{
			editable_gen_info = false;
			$( "#edit_general_info" ).html("Edit this section");
		}
		toggleGenInfo();
	});
	
	function allInputAble(){
	...
	$( "#edit_vehicle_info" ).click(function() {
		if(!editable_veh_info){
			...
			$( "#edit_vehicle_info" ).html("Confirm changes");
		}else{
			editable_veh_info = false;
			$( "#edit_vehicle_info" ).html("Edit this section");
		}
		toggleVehInfo();
	});
	...
	$( "#edit_operating_area" ).click(function() {
		
		editable_operating_are = true;
		toggleOperatingArea();
	});
...
//		});
	...
	$( "#edit_elig" ).click(function() {
		if(!editable_elig){
			editable_elig = true;
			$( "#edit_elig" ).html("Confirm changes");
		}else{
			editable_elig = false;
			$( "#edit_elig" ).html("Edit this section");
		}
		toggleElig();
	});
...
//		});
	...
	$( "#edit_fare" ).click(function() {
		if(!editable_fare){
			editable_fare =  true;
			$( "#edit_fare" ).html("Confirm changes");
		}else{
			editable_fare = false;
			$( "#edit_fare" ).html("Edit this section");
		}
		toggleFare();
	});
...
function toggleGenInfo(){
	if(editable_gen_info){
		$( "input[name='service_name']" ).prop('disabled', false);
	    $( "textarea[name='service_description']" ).prop('disabled', false);
	    $( "textarea[name='how_to_book']" ).prop('disabled', false);
	    $( "select[name='type_of_permit']" ).prop('disabled', false);
	    $( "input[name='cb_not_avail']" ).prop('disabled', false);
	    $( "input[name='not_valid_from']" ).prop('disabled', false);
	    $( "input[name='not_valid_to']" ).prop('disabled', false);
	    $( "input[name='tab_operating_hours_field_cb_weekdays']" ).prop('disabled', false);
	    ...
	    $("button[data-toggle='#service_not_avail']").prop('disabled', false);
	    $("button[data-toggle='#service_not_avail_del']").prop('disabled', false);
	}else{
	    $( "input[name='service_name']" ).prop('disabled', true);
	    $( "textarea[name='service_description']" ).prop('disabled', true);
	    $( "select[name='type_of_permit']" ).prop('disabled', true);
	    ...
	    $("button[data-toggle='#service_not_avail']").prop('disabled', true);
	    $("button[data-toggle='#service_not_avail_del']").prop('disabled', true);
	}
}

var editable_veh_info = false;
function toggleVehInfo(){
	if(editable_veh_info){
		 $( "input[name='regNum']" ).prop('disabled', false);
		 $( "select[name='vehicle_type']" ).prop('disabled', false);
		 $( "#other_vehicle" ).prop('disabled', false);
...
var is_changed_operating_area = false;
var isConfirmed = false;


var editable_operating_are = false;

function toggleOperatingArea(){
	if(editable_operating_are){
		BlitzMap.setEditable(true);
		$( "#div_edit_operating_area_b1" ).hide();
		$( "#div_edit_operating_area_b3" ).fadeIn();
	}else{
		$( "#div_edit_operating_area_b3" ).hide();
		$( "#div_edit_operating_area_b1" ).fadeIn();
	}
}
...
//		    $("select[name='fare_dist1_type']").prop('disabled', false);
//		    $("input[name='fare_dist1_charge']").prop('disabled', false);
		    $( "input[name^='fare_dist']" ).each(function() {
		    	$(this).prop('disabled', false);
			});
		    $( "select[name^='fare_dist']" ).each(function() {
		    	$(this).prop('disabled', false);
			});
		    $("button[data-toggle='#fare_structure']").prop('disabled', false);
		    $("button[data-toggle='#fare_structure_del']").prop('disabled', false);
		    $("input[name='return_fare_multiplier']").prop('disabled', false);
		    $("input[name='discount_for_over60']").prop('disabled', false);
		    $("input[name='discount_for_under16']").prop('disabled', false);
		    $("input[name='discount_for_other_concessionary']").prop('disabled', false);
...
//		    $("input[name='fare_dist1_charge']").prop('disabled', true);
		    ...
		    $("button[data-toggle='#fare_structure']").prop('disabled', true);
		    $("button[data-toggle='#fare_structure_del']").prop('disabled', true);
...
//		console.log("undoChangeMapData: "+$('#undoChangeMapData').val());
//		isConfirmed = true;
...
$( "#undoChanges" ).click(function() {
	...
	BlitzMap.setEditable(true);
	BlitzMap.deleteAll();
	$('#mapData').val($('#undoChangeMapData').val());
	//BlitzMap.setMap( 'myMap', true, 'mapData' );
	BlitzMap.init();
	isConfirmed = false;
	is_changed_operating_area = true;
});
...
function restoreMap(){
	console.log("restore maps");
	BlitzMap.setEditable(true);
	BlitzMap.deleteAll();
	console.log("originalMapData: " + $('#originalMapData').val());
	$('#mapData').val($('#originalMapData').val());
	//BlitzMap.setMap( 'myMap', true, 'mapData' );	BlitzMap.init();
	is_changed_operating_area = false;}
...
$('#collapseOne').on('shown.bs.collapse', function () {
	...
	console.log("highlight collapseOne ");
});
$('#collapseOne').on('hidden.bs.collapse', function () {
	$('a[href="#collapseOne"]').removeClass("highlight");
	console.log("close gen info ");
	var isChanged = isChangedGenInfo();
	if(isChanged){
		if(editable_gen_info){
			$.prompt("General Information has been changed. Do you wish to confirm the changes?", {
				title: "Reminder: General Information has been changed",
				buttons: { "Yes": true, "No": false },
				submit: function(e,v,m,f){
					if(v){
...
$('#collapseTwo').on('shown.bs.collapse', function () {
	...
	console.log("highlight collapseTwo ");
});
$('#collapseTwo').on('hidden.bs.collapse', function () {
	$('a[href="#collapseTwo"]').removeClass("highlight");
	console.log("remove operating area ");
//	var isChanged = isOperatingAreaChanged();
	console.log("is_changed_operating_area: " + is_changed_operating_area);
	if(is_changed_operating_area){
		$.prompt("Operating area has been changed. Do you wish to confirm the changes?", {
			title: "Reminder: Operating area has been changed",
			buttons: { "Yes": true, "No": false },
			submit: function(e,v,m,f){
				if(v){
					is_changed_operating_area = false;
					...
					isOperatingAreaChanged = false;
					// restore map
//					restoreMap();
					setTimeout(function(){restoreMap();}, 1000);
				}
			}
		});
	}
...
$('#collapseThree').on('shown.bs.collapse', function () {
	...
	console.log("highlight collapseThree ");
});
$('#collapseThree').on('hidden.bs.collapse', function () {
	$('a[href="#collapseThree"]').removeClass("highlight");
	console.log("remove collapseThree ");
	var isChanged = isChangedElig();
	if(isChanged){
		if(editable_elig){
			$.prompt("Passenger Eligibility has been changed. Do you wish to confirm the changes?", {
				title: "Reminder: Passenger Eligibility has been changed",
				buttons: { "Yes": true, "No": false },
				submit: function(e,v,m,f){
					if(v){
...
$('#collapseFour').on('shown.bs.collapse', function () {
	...
	console.log("highlight collapseFour ");
});
$('#collapseFour').on('hidden.bs.collapse', function () {
	$('a[href="#collapseFour"]').removeClass("highlight");
	console.log("remove collapseFour ");
	var isChanged = isChangedFare();
	if(isChanged){
		if(editable_fare){
			$.prompt("Fare Structure has been changed. Do you wish to confirm the changes?", {
				title: "Reminder: Fare Structure has been changed",
				buttons: { "Yes": true, "No": false },
				submit: function(e,v,m,f){
					if(v){
...
$('#collapseFive').on('shown.bs.collapse', function () {
	...
	console.log("highlight collapseFive ");
});
$('#collapseFive').on('hidden.bs.collapse', function () {
	$('a[href="#collapseFive"]').removeClass("highlight");
	console.log("remove collapseFive ");
});
$('#collapseSix').on('shown.bs.collapse', function () {
	$('a[href="#collapseSix"]').addClass("highlight");
	console.log("highlight collapseSix ");
});
$('#collapseSix').on('hidden.bs.collapse', function () {
	$('a[href="#collapseSix"]').removeClass("highlight");
	console.log("remove collapseSix ");
	var isChanged = isChangedVehInfo();
	if(isChanged){
		if(editable_veh_info){
			$.prompt("Vehicle information has been changed. Do you wish to confirm the changes?", {
				title: "Reminder: Vehicle Information has been changed",
				buttons: { "Yes": true, "No": false },
				submit: function(e,v,m,f){
					if(v){
					}else{
